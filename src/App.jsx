...
const App = () => {
     ...
     const handleLogin = () => {
          if (loginEmail && loginPassword) {
               // If cart has items, save the current order to history
               if (Object.keys(cart).length > 0) {
                    const newOrder = {
                         id: Date.now(), // Unique ID for the order
                         items: { ...cart }, // Copy of current cart items
                         total: getCartTotal(), // Total cost of the order
                         date: new Date().toLocaleDateString(), // Date of the order
                         tableNumber: tableNumber, // Table number for the order
                    };
                    const newHistory = [newOrder, ...orderHistory]; // Add new order to the beginning of history
                    updateOrderHistory(newHistory);
               }

               setShowLoginModal(false); // Close login modal
               setShowOrderConfirm(true); // Show order confirmation modal
               updateCart({}); // Clear the cart after placing order
          } else {
               setLoginError("Invalid email or password!"); // Show error for invalid login
          }
     };
